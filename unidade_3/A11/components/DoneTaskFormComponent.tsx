...
export default function DoneTaskFormComponent({ navigation }: any) {

    const DoneTask = t.struct({
        name: t.String,
        desc: t.String,
        obs: t.String,
    });
    ...
    const onPressSubmitForm = async () => {
        ...
        try {
            const curr = await AsyncStorage.getItem('DoneTaskData')
            if (!curr) {
                try {
                    await AsyncStorage.setItem('DoneTaskData', JSON.stringify(formData.getValue()))
                    navigation.goBack()
                } catch (error) {
                    console.error(error);
                    Alert.alert(
    ...
    return (
        <View style={ styles.container }>
            <Form type={DoneTask} ref={(c: any) => formData = c} options={ options } />
